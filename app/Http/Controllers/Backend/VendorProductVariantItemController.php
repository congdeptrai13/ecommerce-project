...
class VendorProductVariantItemController extends Controller
{
    public function index(VendorProductVariantItemDataTable $dataTable, $productId, $variantId)
    {
        ...
        return $dataTable->render("vendor.products.variant-item.index", compact('product', 'variant'));
    }
    ...
    public function create($productId, $variantId)
    {
        $product = Product::findOrFail($productId);
        $variant = Variant::findOrFail($variantId);
        return view('vendor.products.variant-item.create', compact('product', 'variant'));
    }
    ...
    public function store(Request $request)
    {
        ...
        return redirect()->route('vendor.product-variant-item.index', ["productId" => $request->productId, 'variantId' => $request->variantId]);
    }

    public function edit($variantItemId)
    {
        $variantItem = VariantItem::findOrFail($variantItemId);
        return view('vendor.products.variant-item.edit', compact('variantItem'));
    }
    ...
    public function update(Request $request, $variantItemId)
    {
        ...
        return redirect()->route('vendor.product-variant-item.index', ["productId" => $VariantItem->variant->product_id, 'variantId' => $VariantItem->variant->id]);
    }
